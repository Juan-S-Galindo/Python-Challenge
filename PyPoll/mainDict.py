...
def PyPoll(row) : #PyPoll function to print the final results and all the mumbo jumbo
    ...
    candidateIndex = row[2] #Declares the variable candiadate  index as the value under the column with the names of the candidates.
    totalVotes += 1 #Adds 1 vote to the total tally every time we do a row iteration.

    if candidateIndex in candidatesDict : #If the candidate's name is in the keys of the  candidate global dictionary.

        candidatesDict[candidateIndex] += 1  #Adds 1 vote to the value of the key of the respective candidate.

    else: #If the name of the candidate is not in the dictionary

        candidatesDict.update({candidateIndex:1}) #Takes the candidate global dictionary, creates a new key with the name of the candidate and sets the value to 1 to account for the first vote.

    keyMax = max(candidatesDict, key=candidatesDict.get) #Variable to return the key with  the highest value.

    if (totalVotes+1 == csvTotalLines): #if total votes + 1 to account for the header row is equal to the total lines in the csv print the results.
...
fileName = os.path.join('Resources','election_data.csv') #Opens the CSV file.
...
with open(fileName, 'r') as csvfile: #Opens the file with reader settings as csvfile.
    ...
    csvTotalLines = len(list(open(fileName))) #Returns the total lines in the CSV file.
    csvheader = next(csvreader,None) #Csvheader allows us to skip the header in the calculations. 
