...
with open(fileName, 'r') as csvfile: #Opens the file with reader settings as csvfile.
    ...
    for row in csvreader: #For every row in the csvreader
        ...
        if candidateIndex in candidatelist : #If the candidate's name is the list called candidatelist.

            candidateVotes[candidatelist.index(candidateIndex)] += 1 #Finds the index of the candidate in the candadidate list and returns the index to the candidate votes and adds 1 to the index.
            
            if candidateVotes[candidatelist.index(candidateIndex)] > winnerVotes: #Keeps track of the candidate with the most votes.
                winnerVotes = candidateVotes[candidatelist.index(candidateIndex)] #Tracks the highest vote count.
        ...
        if (totalVotes+1 == csvTotalLines): #If the total number of votes +1 to account for the title of the file are the same as teh total csvlines.
            s='-'*20
